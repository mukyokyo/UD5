...
const uint8_t COLED::ucFont[14*48] = {
...
const uint8_t COLED::ucBigFont[16*384] = {
...
const uint8_t COLED::ucSmallFont[5*96] = {
...
int COLED::I2CWrite (CI2C **pI2C, uint8_t iAddr, uint8_t *pData, int iLen) {
  if ((*pI2C) != NULL) {
    (*pI2C)->lock_sem();
    (*pI2C)->begin (iAddr);
    for (int i = 0; i < iLen; i++) (*pI2C)->write (pData[i]);
    (*pI2C)->end();
    (*pI2C)->unlock_sem();
...
uint8_t COLED::I2CTest (CI2C **pI2C, uint8_t addr) {
  bool b = false;
  if ((*pI2C) != NULL) {
    (*pI2C)->lock_sem();
    if ((*pI2C)->begin(addr)) b = (*pI2C)->end();
    (*pI2C)->begin (addr, true);
    (*pI2C)->unlock_sem();
    return b;
  }
  return b;
...
int COLED::I2CReadRegister (CI2C **pI2C, uint8_t iAddr, uint8_t u8Register, uint8_t *pData, int iLen) {
  int r = 0;
  if ((*pI2C) != NULL) {
    (*pI2C)->lock_sem();
    if ((*pI2C)->begin (iAddr)) {
      (*pI2C)->write (u8Register);
      if ((*pI2C)->begin (iAddr, true)) {
        r = (*pI2C)->read (pData, iLen) ? 1 : 0;
      }
    }
    (*pI2C)->end();
    (*pI2C)->unlock_sem();
...
int COLED::I2CRead (CI2C **pI2C, uint8_t iAddr, uint8_t *pData, int iLen) {
  int r = 0;
  if ((*pI2C) != NULL) {
    (*pI2C)->lock_sem();
    if ((*pI2C)->begin (iAddr, true)) {
...
void COLED::_I2CWrite (SSOLED *pOLED, uint8_t *pData, int iLen) {
  I2CWrite (&pOLED->bbi2c, pOLED->oled_addr, pData, iLen);
}

// Send a single byte command to the OLED controller
void COLED::oledWriteCommand (SSOLED *pOLED, uint8_t c) {
  uint8_t buf[2];
...
void COLED::oledWriteCommand2 (SSOLED *pOLED, uint8_t c, uint8_t d) {
  uint8_t buf[3];
...
#ifdef FUTURE
void COLED::oledCachedFlush (void) {
  _I2CWrite (bCache, bEnd); // write the old data
#ifdef USE_BACKBUFFER
  __aeabi_memcpy (&ucScreen[iScreenOffset], &bCache[1], bEnd - 1);
  iScreenOffset += (bEnd - 1);
#endif
  bEnd = 1;
}

void COLED::oledCachedWrite (uint8_t *pData, uint8_t bLen) {

  if (bEnd + bLen > MAX_CACHE) { // need to flush it
    oledCachedFlush(); // write the old data
  }
  __aeabi_memcpy (&bCache[bEnd], pData, bLen);
  bEnd += bLen;
}
#endif // FUTURE
...
COLED::COLED (SSOLED *pOLED, CI2C *dev, int iType, int iAddr, int bFlip, int bInvert, uint8_t *pBuffer) {
  begin(pOLED, dev, iType, iAddr, bFlip, bInvert, pBuffer);
}
...
int COLED::begin (SSOLED *pOLED, CI2C *dev, int iType, int iAddr, int bFlip, int bInvert, uint8_t *pBuffer) {
  uint8_t uc[4];
  pOLED->rc = OLED_NOT_FOUND;
  ...
  if (iAddr == -1 || iAddr == 0 || iAddr == 0xff) { // find it
    ...
    if (I2CTest (&pOLED->bbi2c, 0x3c)) {
      ...
      return pOLED->rc; // no display found!
    }
  } else {
  ...
  if (u == 0x7 || u == 0xf) { // SH1107
    ...
    pOLED->rc = OLED_SH1107_3C;
    bFlip = !bFlip; // SH1107 seems to have this reversed from the usual direction
  } else if (u == 0x8) { // SH1106
    pOLED->rc = OLED_SH1106_3C;
    pOLED->oled_type = OLED_132x64; // needs to be treated a little differently
  } else if (u == 3 || u == 6) { // 6=128x64 display, 3=smaller
    pOLED->rc = OLED_SSD1306_3C;
  }
  if (pOLED->oled_addr == 0x3d)
    pOLED->rc++; // return the '3D' version of the type

  if (iType == OLED_128x32 || iType == OLED_96x16)
    _I2CWrite (pOLED, (uint8_t *)oled32_initbuf, sizeof (oled32_initbuf));
  else if (iType == OLED_128x128)
    _I2CWrite (pOLED, (uint8_t *)oled128_initbuf, sizeof (oled128_initbuf));
  else if (iType == OLED_72x40)
    _I2CWrite (pOLED, (uint8_t *)oled72_initbuf, sizeof (oled72_initbuf));
  else // 132x64, 128x64 and 64x32
    _I2CWrite (pOLED, (uint8_t *)oled64_initbuf, sizeof (oled64_initbuf));
  if (bInvert) {
    uc[0] = 0; // command
...
int COLED::get_rc (SSOLED *pOLED) {
  return pOLED->rc;
}

// Power up/down the display
// useful for low power situations
void COLED::Power (SSOLED *pOLED, uint8_t bOn) {
  oledWriteCommand (pOLED, bOn ? 0xaf : 0xae);
}

// Sets the brightness (0=off, 255=brightest)
void COLED::SetContrast (SSOLED *pOLED, uint8_t ucContrast) {
...
int COLED::ScrollBuffer (SSOLED *pOLED, int iStartCol, int iEndCol, int iStartRow, int iEndRow, int bUp) {
  uint8_t b, *s;
  int col, row;
...
void COLED::oledSetPosition (SSOLED *pOLED, int x, int y, int bRender) {
  uint8_t buf[4];
...
void COLED::oledWriteDataBlock (SSOLED *pOLED, const uint8_t *ucBuf, int iLen, int bRender) {
  uint8_t ucTemp[129];
  ...
  if (bRender) {
    __aeabi_memcpy (&ucTemp[1], ucBuf, iLen);
    _I2CWrite (pOLED, ucTemp, iLen + 1);
  }
  // Keep a copy in local buffer
  if (pOLED->ucScreen) {
    __aeabi_memcpy (&pOLED->ucScreen[pOLED->iScreenOffset], ucBuf, iLen);
...
void COLED::oledWriteFlashBlock (SSOLED *pOLED, uint8_t *s, int iLen) {
  int j;
...
void COLED::oledRepeatByte (SSOLED *pOLED, uint8_t b, int iLen) {
  int j;
  int iWidthMask = pOLED->oled_x - 1;
...
uint8_t *COLED::PlayAnimFrame (SSOLED *pOLED, uint8_t *pAnimation, uint8_t *pCurrent, int iLen) {
  uint8_t *s;
  int i, j;
  uint8_t b, bCode;
...
void COLED::DrawSprite (SSOLED *pOLED, uint8_t *pSprite, int cx, int cy, int iPitch, int x, int y, uint8_t iPriority) {
...
void COLED::DrawTile (SSOLED *pOLED, const uint8_t *pTile, int x, int y, int iRotation, int bInvert, int bRender) {
...
int COLED::SetPixel (SSOLED *pOLED, int x, int y, uint8_t ucColor, int bRender) {
  int i;
  uint8_t uc, ucOld;
...
// Invert font data
void COLED::InvertBytes (uint8_t *pData, uint8_t bLen) {
  uint8_t i;
  for (i = 0; i < bLen; i++) {
    *pData = ~ (*pData);
    pData++;
...
int COLED::LoadBMP (SSOLED *pOLED, const uint8_t *pBMP, int bInvert, int bRender) {
  int16_t i16;
...
void COLED::SetCursor (SSOLED *pOLED, int x, int y) {
  pOLED->iCursorX = x;
  pOLED->iCursorY = y;
}

// Turn text wrap on or off for the oldWriteString() function
void COLED::SetTextWrap (SSOLED *pOLED, int bWrap) {
...
//
//  Returns 0 for success, -1 for invalid parameter
int COLED::WriteString (SSOLED *pOLED, int iScroll, int x, int y, const char *szMsg, int iSize, int bInvert, int bRender) {
  int i, iFontOff, iLen, iFontSkip;
  uint8_t c, *s, ucTemp[40];
  ...
  if (iSize == FONT_8x8) { // 8x8 font
    ...
    while (pOLED->iCursorX < pOLED->oled_x && szMsg[i] != 0 && pOLED->iCursorY < pOLED->oled_y / 8) {
      if (iScroll < 8) { // only display visible characters
        ...
        ucTemp[0] = 0;
        __aeabi_memcpy (&ucTemp[1], &ucFont[iFontOff], 7);
...
//      oledCachedFlush(); // write any remaining data
    ...
    while (pOLED->iCursorX < pOLED->oled_x && pOLED->iCursorY < (pOLED->oled_y / 8) - 1 && szMsg[i] != 0) {
      ...
      if (iScroll < 12) { // if characters are visible
        int tx, ty;
        c = szMsg[i] - 32;
        uint8_t uc1, uc2, ucMask, *pDest;
        s = (uint8_t *)&ucSmallFont[ (int)c * 5];
        ucTemp[0] = 0; // first column is blank
        __aeabi_memcpy (&ucTemp[1], s, 6);
        if (bInvert)
    ...
    while (pOLED->iCursorX < pOLED->oled_x && pOLED->iCursorY < (pOLED->oled_y / 8) - 1 && szMsg[i] != 0) {
      ...
      if (iScroll < 16) { // if characters are visible
        ...
        c = szMsg[i] - 32;
        uint8_t uc1, uc2, ucMask, *pDest;
        s = (uint8_t *)&ucFont[ (int)c * 7];
        ucTemp[0] = 0;
        __aeabi_memcpy (&ucTemp[1], s, 7);
        if (bInvert)
    ...
    while (pOLED->iCursorX < pOLED->oled_x && pOLED->iCursorY < (pOLED->oled_y / 8) && szMsg[i] != 0) {
      if (iScroll < 6) { // if characters are visible
        ...
        // we can't directly use the pointer to FLASH memory, so copy to a local buffer
        ucTemp[0] = 0;
        __aeabi_memcpy (&ucTemp[1], &ucSmallFont[ (int)c * 5], 5);
...
// returns 0 for success, -1 for invalid parameter
int COLED::DrawGFX (SSOLED *pOLED, uint8_t *pBuffer, int iSrcCol, int iSrcRow, int iDestCol, int iDestRow, int iWidth, int iHeight, int iSrcPitch) {
  int y;
  ...
  for (y = iSrcRow; y < iSrcRow + iHeight; y++) {
    const uint8_t *s = &pBuffer[ (y * iSrcPitch) + iSrcCol];
...
void COLED::DumpBuffer (SSOLED *pOLED, uint8_t *pBuffer) {
  int x, y;
  int iLines, iCols;
  uint8_t bNeedPos;
...
void COLED::Fill (SSOLED *pOLED, uint8_t ucData, int bRender) {
  uint8_t x, y;
...
void COLED::SetBackBuffer (SSOLED *pOLED, uint8_t *pBuffer) {
  pOLED->ucScreen = pBuffer;
}

// Draw a line between 2 points
void COLED::DrawLine (SSOLED *pOLED, int x1, int y1, int x2, int y2, int bRender) {
  int temp;
  int dx = x2 - x1;
  int dy = y2 - y1;
  int error;
...
int COLED::ScaledString (SSOLED *pOLED, int x, int y, char *szMsg, int iSize, int bInvert, int iXScale, int iYScale, int iRotation) {
  ...
  while (*szMsg) {
    ...
    // we can't directly use the pointer to FLASH memory, so copy to a local buffer
    ucTemp[0] = 0; // first column is blank
    __aeabi_memcpy (&ucTemp[1], &s[iFontOff], iFontWidth - 1);
...
void COLED::DrawScaledPixel (SSOLED *pOLED, int iCX, int iCY, int x, int y, int32_t iXFrac, int32_t iYFrac, uint8_t ucColor) {
  uint8_t *d, ucMask;
...
void COLED::DrawScaledLine (SSOLED *pOLED, int iCX, int iCY, int x, int y, int32_t iXFrac, int32_t iYFrac, uint8_t ucColor) {
  int iLen, x2;
  uint8_t *d, ucMask;
  if (iXFrac != 0x10000) x = ((x * iXFrac) >> 16);
  if (iYFrac != 0x10000) y = ((y * iYFrac) >> 16);
...
void COLED::BresenhamCircle (SSOLED *pOLED, int iCX, int iCY, int x, int y, int32_t iXFrac, int32_t iYFrac, uint8_t ucColor, uint8_t bFill) {
...
void COLED::Ellipse (SSOLED *pOLED, int iCenterX, int iCenterY, int32_t iRadiusX, int32_t iRadiusY, uint8_t ucColor, uint8_t bFilled) {
...
void COLED::Rectangle (SSOLED *pOLED, int x1, int y1, int x2, int y2, uint8_t ucColor, uint8_t bFilled) {
...
void COLED::reverse (char *s, int len) {
  int i = 0, j = len - 1, tmp;
  while (i < j) {
    tmp = s[i];
    s[i] = s[j];
...
int COLED::i2s (int x, char *s, int digit) {
  int i = 0;
  bool sign = (x < 0);
  x = abs (x);
  while (x) {
...
// raising a power
int COLED::mypower (int nb, int power) {
  if (power < 0) return (0);
  else if (power == 1) return (nb);
  else return (nb * mypower (nb, power - 1));
}

// Conversion from double to string (simplified version)
char *COLED::d2s (char *s, double n, int decinum) {
  // integer part
  int int_part = (int)n;
  int i = i2s (int_part, s, 0);
  // decimal part
  double deci_part = n - (double)int_part;
  if (decinum != 0) {
    s[i] = '.';
    deci_part = deci_part * (double)mypower (10, decinum);
    i2s (abs (deci_part), s + i + 1, decinum);
  }
  return s;
}

void COLED::putc (SSOLED *pOLED, char c) {
  char s[2] = { c, '\0' };
  if (c == '\n') pOLED->iCursorY++;
  else if (c == '\r') pOLED->iCursorX = 0;
  else WriteString(pOLED, 0, -1, -1, s, COLED::FONT_SMALL, 0, 1);
}

void COLED::puts (SSOLED *pOLED, char *s) {
  while (*s) putc (pOLED, *s++);
}

int COLED::printf (SSOLED *pOLED, const char *fmt, ...) {
  int len = 0;
  ...
  for (;;) {
    ...
    switch (d) {        /* Type is... */
      ...
      case 'C' :          /* Character */
        len++;
        putc (pOLED, (char)va_arg (arp, int));
        continue;
      case 'B' :          /* Binary */
        r = 2;
        break;
      ...
      case 'X' :          /* Hexdecimal */
        ...
        break;
      case 'F':
        puts (pOLED, d2s (p = s, va_arg (arp, double), w));
        continue;
